...
#line 90 "./src/objects/database.lzz"
                ...
                NODE_SET_METHOD(exports, "fstream", JS_fstream);
...
#line 373 "./src/objects/database.lzz"
                ...
                if (src == -1 || dst == -1) {
                        if (src != -1) close(src);
                        if (dst != -1) close(dst);
                        ThrowTypeError("Failed to open file descriptors");
                } else {
                        do len = read(src, buf, BUFFER_SIZE);
                        while (len > 0 && write(dst, buf, len) == len);
                        if (static_cast<int>(len != 0) | close(src) | close(dst)) {
                                ThrowTypeError("Failed to run read/write");
                        }
                }
}
...
#line 395 "./src/objects/database.lzz"
                ...
                if (src == NULL || dst == NULL) {
                        ...
                        ThrowTypeError("Failed to open files");
                } else {
                        do len = fread(buf, PAGE_SIZE, PAGES_PER_BUFFER, src);
                        while (len && fwrite(buf, PAGE_SIZE, len, dst) == len);
                        if (ferror(src) | ferror(dst) | fclose(src) | fclose(dst)) {
                                ThrowTypeError("Failed to run fread/fwrite");
                        }
                }
}
#line 417 "./src/objects/database.lzz"
void Database::JS_fstream (v8::FunctionCallbackInfo <v8 :: Value> const & info)
#line 417 "./src/objects/database.lzz"
                                {
                if ( info . Length ( ) <= ( 0 ) || ! info [ 0 ] -> IsString ( ) ) return ThrowTypeError ( "Expected " "first" " argument to be " "a string" ) ; v8 :: Local < v8 :: String > string1 = v8 :: Local < v8 :: String > :: Cast ( info [ 0 ] ) ; if ( info . Length ( ) <= ( 1 ) || ! info [ 1 ] -> IsString ( ) ) return ThrowTypeError ( "Expected " "second" " argument to be " "a string" ) ; v8 :: Local < v8 :: String > string2 = v8 :: Local < v8 :: String > :: Cast ( info [ 1 ] ) ; v8 :: String :: Utf8Value utf81 ( EXTRACT_STRING ( info . GetIsolate ( ) , string1 ) ) ; v8 :: String :: Utf8Value utf82 ( EXTRACT_STRING ( info . GetIsolate ( ) , string2 ) ) ; const char * src_name = * utf81 ; const char * dst_name = * utf82 ;
                ...
                if (!src.good() || !dst.good()) {
                        ThrowTypeError("Failed to open fstreams");
                } else {
                        dst << src.rdbuf();
                        src.close();
                        dst.close();
                        if (src.fail() || dst.fail()) {
                                ThrowTypeError("Failed to run ofstream << ifstream");
                        }
                }
}
